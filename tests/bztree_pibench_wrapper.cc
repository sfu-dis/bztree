...
#ifdef PMDK
    pmwcas::InitLibrary(pmwcas::PMDKAllocator::Create(opt.pool_path.c_str(), "bztree_layout", opt.pool_size),
...
bool bztree_wrapper::find(const char* key, size_t key_sz, char *value_out) {
    ...
    return tree_->Read(reinterpret_cast<const char*>(&k), key_sz, (uint64_t *)value_out).IsOk();
}

bool bztree_wrapper::insert(const char* key, size_t key_sz, const char* value, size_t value_sz) {
    // FIXME(tzwang): for now only support 8-byte values
...
bool bztree_wrapper::update(const char* key, size_t key_sz, const char* value, size_t value_sz) {
    // FIXME(tzwang): for now only support 8-byte values
    assert(value_sz == sizeof(uint64_t));
    uint64_t k = __builtin_bswap64(*reinterpret_cast<const uint64_t*>(key));
    uint64_t v = *reinterpret_cast<uint64_t*>(const_cast<char*>(value));
...
int bztree_wrapper::scan(const char* key, size_t key_sz, int scan_sz, char*& values_out) {
    ...
    auto iter = tree_->RangeScanBySize(reinterpret_cast<const char*>(&k), key_sz, scan_sz);
    for(scanned=0; (scanned < scan_sz); ++scanned)
    {
        ...
        uint64_t result_key = __builtin_bswap64(*reinterpret_cast<const uint64_t*>(record->GetKey()));
